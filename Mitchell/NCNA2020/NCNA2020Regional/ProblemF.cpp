...
//connection, weight
// for args: cur node, past node, cur max
pll dfs(ll c, ll p, ll m){
    ll _m = m;
    ll _c = c;
    for(pll cur: tree[c]){
        if(cur.first == p) continue;
        pll t = dfs(cur.first, c, m + cur.second);
        if(t.second > _m){
            _m = t.second;
            _c = t.first;
        }
    }
    return make_pair(_c, _m);
}
...
int main(){
    ...
    repi(N-1){
        cin>>n1>>n2>>w;
        tree[n1].pb(make_pair(n2, w));
        tree[n2].pb(make_pair(n1, w));
    }
    pll cur = dfs(n1, -1, 0);
    cur = dfs(cur.first, -1, 0);
    cout<<cur.second;
    return 0;
}
