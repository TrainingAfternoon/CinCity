...
struct villager{
    ...
    bool searched;
    int vouch;
    V<villager*> connections;

    villager(int v) : val(v), vouch(0), searched(false){}
    villager() : val(1), searched(false){}

    void count(){
        this->searched = true;
        for(villager* vil : this->connections){
            if(!(vil->searched)){
                vil->vouch += 1;
                vil->count();
            }
...
//            cout<<"\tconnects to "<<v[i].connections[j].val<<nn;
//        }
//   }
...
//    for(int i = 0; i < n; i++){
//        for(villager *vil : v[i]->connections){
//            //if(vil->val != i)
//                count[vil->val] += 1; 
//        }
//    }
//    for(int i = 0; i < n; i++) cout<<(count[i]>k)<<nn;
